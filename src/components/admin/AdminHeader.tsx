...
const notificationColors: Record<string, string> = {
  approval: "text-success bg-success/10", message: "text-info bg-info/10", meeting: "text-primary bg-primary/10", alert: "text-destructive bg-destructive/10", user: "text-muted-foreground bg-muted/50",
};
const notificationRoutes: Record<string, string> = {
  approval: "/approvals", message: "/messages", meeting: "/meetings", alert: "/security", user: "/influencers",
};
  ...
  return (
    <header className="h-16 border-b border-border/50 bg-card/60 backdrop-blur-xl flex items-center justify-between px-4 lg:px-6 sticky top-0 z-30">
      ...
      <div className="flex items-center gap-2">
        {/* Clock */}
        <div className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-muted/30 border border-border/30 text-sm">
          <Clock className="w-3.5 h-3.5 text-primary" />
          <span className="font-mono text-xs text-foreground">{tehranTime}</span>
          <span className="text-[10px] text-muted-foreground">{t("تهران", "Tehran")}</span>
        </div>
        ...
        <div className="relative">
          <button onClick={() => setNotifOpen(!notifOpen)} className="relative p-2.5 rounded-xl hover:bg-muted transition-colors">
              ...
              <div className="absolute end-0 top-12 w-80 sm:w-96 bg-card border border-border/50 rounded-2xl shadow-2xl z-50 overflow-hidden animate-fade-in">
                <div className="flex items-center justify-between px-4 py-3 border-b border-border/30">
                  <div className="flex items-center gap-2">
                    ...
                    {unreadCount > 0 && <span className="min-w-[20px] h-5 rounded-full bg-primary/10 text-primary text-xs font-bold flex items-center justify-center px-1.5">{unreadCount}</span>}
                  </div>
                  <div className="flex items-center gap-1">
                    {unreadCount > 0 && <button onClick={markAllRead} className="text-xs text-primary hover:underline px-2 py-1">{t("خواندن همه", "Mark all read")}</button>}
                    <button onClick={() => setNotifOpen(false)} className="p-1 rounded-lg hover:bg-muted transition-colors"><X className="w-4 h-4 text-muted-foreground" /></button>
                  </div>
                </div>
                <div className="max-h-[400px] overflow-y-auto scrollbar-thin">
                  {notifications.map((notif) => {
                    ...
                    return (
                      <button key={notif.id} onClick={() => handleNotifClick(notif)} className={cn("w-full flex items-start gap-3 px-4 py-3 text-start hover:bg-muted/30 transition-colors border-b border-border/10 last:border-0", !notif.read && "bg-primary/[0.03]")}>
                        <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center shrink-0 mt-0.5", colorClass)}><Icon className="w-4 h-4" /></div>
                ...
                <div className="border-t border-border/30 px-4 py-2.5">
                  <button onClick={() => { setNotifOpen(false); navigate("/security"); }} className="w-full text-center text-xs text-primary hover:underline font-medium py-1">{t("مشاهده همه اعلان‌ها", "View all notifications")}</button>
                </div>
              </div>
            </>
          )}
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button size="sm" className="hidden sm:flex gap-1.5 gold-gradient text-primary-foreground font-semibold rounded-xl hover:opacity-90 transition-opacity border-0">
              <Plus className="w-4 h-4" />{t("عملیات", "Actions")}
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="w-56 bg-card border-border/50 rounded-xl">
            <DropdownMenuItem className="gap-2 cursor-pointer rounded-lg" onClick={() => navigate("/security")}><UserPlus className="w-4 h-4 text-primary" />{t("افزودن کارمند", "Add Staff")}</DropdownMenuItem>
            <DropdownMenuItem className="gap-2 cursor-pointer rounded-lg" onClick={() => navigate("/campaigns")}><Megaphone className="w-4 h-4 text-primary" />{t("ایجاد کمپین", "Create Campaign")}</DropdownMenuItem>
            <DropdownMenuItem className="gap-2 cursor-pointer rounded-lg" onClick={() => navigate("/approvals")}><ClipboardList className="w-4 h-4 text-primary" />{t("بررسی درخواست‌ها", "Review Requests")}</DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem className="gap-2 cursor-pointer rounded-lg" onClick={() => navigate("/messages")}><Send className="w-4 h-4 text-primary" />{t("ارسال پیام عمومی", "Broadcast Message")}</DropdownMenuItem>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 p-1.5 rounded-xl hover:bg-muted transition-colors">
              <div className="w-8 h-8 rounded-lg gold-gradient flex items-center justify-center text-xs font-bold text-primary-foreground">A</div>
